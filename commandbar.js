...
(function() {
    ...
    const fetchProduct = async (currentValue, callback) => {
        ...
        requestData.body = "{\"jsonrpc\":\"2.0\",\"method\":\"call\",\"params\":{\"model\":\"product.template\",\"domain\":[\"&\",[\"type\",\"in\",[\"consu\",\"product\"]],\"|\",\"|\",\"|\",\"|\",[\"default_code\",\"ilike\",\""+ currentValue +"\"],[\"product_variant_ids.default_code\",\"ilike\",\""+ currentValue +"\"],[\"name\",\"ilike\",\""+ currentValue +"\"],[\"barcode\",\"ilike\",\""+ currentValue +"\"],[\"sku\",\"ilike\",\""+ currentValue +"\"]],\"fields\":[\"id\",\"product_variant_count\",\"currency_id\",\"activity_state\",\"name\",\"default_code\",\"lst_price\",\"qty_available\",\"uom_id\",\"standard_price\",\"type\"],\"limit\":80,\"sort\":\"\",\"context\":{\"lang\":\"en_GB\",\"tz\":\"Europe/Amsterdam\",\"uid\":101,\"allowed_company_ids\":[1],\"params\":{\"action\":498,\"cids\":1,\"menu_id\":307,\"model\":\"product.template\",\"view_type\":\"kanban\"},\"default_type\":\"product\",\"quantity_available_locations_domain\":[\"internal\"],\"bin_size\":true}},\"id\":884688463}";

        let productData = await fetch(baseUrl, requestData);
        let jsonData = await productData.json();

        if (jsonData.result.records.length !== 0) {
            toUrl(jsonData.result.records[0].id, 498, "product.template", 307);
        } else {
